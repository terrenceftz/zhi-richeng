...
check_login() {
    if [ ! -f /tmp/feishu_token.txt ] || [ ! -s /tmp/feishu_token.txt ]; then
        ...
        token=$(curl -s -X POST https://your-server.example.com/api/auth/login \
          -H "Content-Type: application/json" \
          -d '{"email":"admin@zhi.com","password":"your-password"}' | python3 -c "
import sys, json
try:
...
show_current_config() {
    ...
    curl -s https://your-server.example.com/api/settings \
...
configure_feishu_app() {
    ...
    response=$(curl -s -X POST https://your-server.example.com/api/settings \
      -H "Content-Type: application/json" \
...
bind_openid() {
    ...
    response=$(curl -s -X POST https://your-server.example.com/api/settings \
      ...
      -H "Authorization: Bearer $TOKEN" \
      -d "{
        \"feishu_openid_<你的OpenID>\": \"$OPEN_ID\"
      }")
...
test_feishu() {
    ...
    response=$(curl -s -X POST https://your-server.example.com/api/tasks \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer $TOKEN" \
      -d '{
        "title": "飞书机器人测试任务",
