...
export function EmptyState({ title, hint, icon }: { title: string; hint?: string; icon?: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
      <div className="relative flex items-center justify-center rounded-full bg-slate-100 p-3 text-slate-400 dark:bg-slate-800 dark:text-slate-500">
        {isKirby ? (
          <img src={KIRBY_STICKERS.kirbySit} alt="" aria-hidden className="h-7 w-7 object-contain" />
        ) : (
